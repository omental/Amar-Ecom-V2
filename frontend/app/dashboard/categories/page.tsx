...
type Category = { id: string; name: string; slug: string; description: string | null; storefront_template_id?: string | null };
type CategoryForm = { name: string; slug: string; description: string; storefront_template_id: string };
const emptyForm: CategoryForm = { name: "", slug: "", description: "", storefront_template_id: "" };
...
export default function CategoriesPage() {
  ...
  useEffect(() => { let mounted = true; Promise.all([api.get<Category[]>("/categories?skip=0&limit=100"), api.get<OnlineStoreTheme[]>("/admin/storefront/themes")]).then(([data, themeData]) => { if (mounted) { setCategories(data); setTemplates(themeData.filter((theme) => theme.status === "published").flatMap((theme) => theme.templates).filter((template) => template.resource_type === "collection")); } }).catch((reason) => { if (mounted) setError(reason instanceof ApiError ? reason.message : "Failed to load categories"); }).finally(() => { if (mounted) setLoading(false); }); return () => { mounted = false; }; }, []);
  ...
  function openEdit(item: Category) { const next = { name: item.name, slug: item.slug, description: item.description || "", storefront_template_id: item.storefront_template_id || "" }; setEditing(item); setForm(next); setBaseline(JSON.stringify(next)); setModalError(""); setModalOpen(true); }
  async function submit(event: React.FormEvent<HTMLFormElement>) { event.preventDefault(); setPending(true); setModalError(""); try { const payload = { ...form, description: form.description || null, storefront_template_id: form.storefront_template_id || null }; if (editing) await api.patch(`/categories/${editing.id}`, payload); else await api.post("/categories", payload); await load(); setModalOpen(false); setSuccess(editing ? "Category updated successfully." : "Category created successfully."); } catch (reason) { setModalError(reason instanceof ApiError ? reason.message : "Failed to save category"); } finally { setPending(false); } }
  ...
  return <div className="space-y-4">
    ...
    {editing ? <CustomFieldsEditor ownerType="collection" ownerId={editing.id} /> : null}
    {modalOpen ? <ControlModal title={editing ? "Edit Category" : "Add Category"} description="Use a clear name, unique slug, and optional storefront template." onClose={() => setModalOpen(false)} size="sm" dirty={JSON.stringify(form) !== baseline}><form onSubmit={submit} className="space-y-4"><FormField label="Name" required><input name="name" value={form.name} onChange={(event) => setForm((current) => ({ ...current, name: event.target.value }))} className={inputClass} /></FormField><FormField label="Slug" description="Must be unique." required><input name="slug" value={form.slug} onChange={(event) => setForm((current) => ({ ...current, slug: event.target.value }))} className={inputClass} /></FormField><FormField label="Description"><textarea name="description" rows={4} value={form.description} onChange={(event) => setForm((current) => ({ ...current, description: event.target.value }))} className={inputClass} /></FormField><FormField label="Storefront template" description="The default collection template is used when no override is selected."><select value={form.storefront_template_id} onChange={(event) => setForm((current) => ({ ...current, storefront_template_id: event.target.value }))} className={inputClass}><option value="">Default collection template</option>{templates.map((template) => <option key={template.id} value={template.id}>{template.name}</option>)}</select></FormField>{modalError ? <ErrorAlert message={modalError} persistent /> : null}<FormActions pending={pending} onCancel={() => setModalOpen(false)} saveLabel={editing ? "Save Category" : "Create Category"} pendingLabel="Saving..." sticky /></form></ControlModal> : null}
  </div>;
}
