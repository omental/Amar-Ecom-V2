...
const legacyCapabilities: Record<keyof LegacyPermissions, Capability[]> = {
  ...
  inventory: ["inventory.view", "inventory.create", "inventory.update", "inventory.delete", "products.view", "products.create", "products.update", "products.delete", "categories.view", "brands.view", "warehouses.view", "stock_movements.view", "suppliers.view", "purchase_orders.view"],
  crm: ["customers.view", "customers.create", "customers.update", "customers.delete"],
  logistics: ["logistics.view", "shipments.view", "shipments.create", "shipments.update", "shipments.delete", "couriers.view", "courier_integrations.view"],
...
export function resolveCapabilities(user?: AuthUser | null): Set<string> {
  ...
  const resolved = new Set(user.permissions ?? []);
