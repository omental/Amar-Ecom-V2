...
| `/dashboard/woocommerce` | Working | `/api/v1/woocommerce/settings`, `/api/v1/woocommerce/test-connection`, `/api/v1/woocommerce/products-preview`, `/api/v1/woocommerce/products-import`, `/api/v1/woocommerce/orders-preview`, `/api/v1/woocommerce/orders-import`, `/api/v1/woocommerce/sync-logs`, `/api/v1/woocommerce/sync-logs/{id}` | Manual import hardening only; encrypted server-side credentials, duplicate-aware previews, row-level import summaries, log detail/filtering, clear empty states, and manual-read-only safety messaging are present, but there is still no auto-sync, no write-back to WooCommerce, and no destructive updates. |
