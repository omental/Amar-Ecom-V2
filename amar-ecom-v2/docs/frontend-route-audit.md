...
| `/dashboard/orders` | Working | `/api/v1/orders`, `/api/v1/orders/operations-summary`, `/api/v1/orders/dispatch-export`, `/api/v1/orders/duplicate-check`, `/api/v1/orders/batch-actions`, supporting customer/warehouse/shipment data | Phase 14A turns this into a stronger operations cockpit, and Phase 14B finishes the operator flow with row selection, quick filters, filtered or dispatch CSV export, batch mark-printed, copy-print-links support, and safe batch status updates while keeping Woo refresh manual and safe. |
...
| `/dashboard/logistics` | Working | `/api/v1/logistics/pending-dispatch`, `/api/v1/logistics/operations-summary`, `/api/v1/logistics/reconciliation-export`, `/api/v1/orders`, `/api/v1/shipments`, `/api/v1/couriers` | Internal dispatch and reconciliation workspace now includes operations cards for pending dispatch, courier handoff, missing tracking, external failure visibility, status-sync follow-up, filtered reconciliation totals, and CSV export while remaining manual-first and non-destructive. |
| `/dashboard/couriers` | Working | `/api/v1/couriers` | Internal master-data only. |
| `/dashboard/shipments` | Working | `/api/v1/shipments`, `/api/v1/shipments/batch-status-update`, `/api/v1/couriers`, `/api/v1/orders` | Reconciliation works, shipment rows surface external courier metadata when available, and Phase 14B adds quick filters, row selection, CSV export, and safe internal-only batch status updates without calling courier APIs. |
