...
| WooCommerce | `/api/v1/woocommerce` | `GET /settings`, `PATCH /settings`, `POST /test-connection`, `GET /products-preview`, `POST /products-import`, `GET /orders-preview`, `POST /orders-import`, `GET /sync-logs`, `GET /sync-logs/{id}` | Protected | Admin-only manual import hardening plus QA polish. Credentials are stored encrypted server-side, previews expose duplicate indicators, imports return row-level outcomes, logs support filtering/detail, endpoint errors are normalized for common WooCommerce failures, and the integration remains read-only against WooCommerce with no auto-sync, no destructive updates, and no push-back. |
